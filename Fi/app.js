...
app.get('/p', (req,res)=>{
    res.locals.products = req.session.products;
    let curp = products[req.query.pnumber];
...
app.get('/cart', (req,res)=>{
    ...
    items.reduce(function(res, value) {
        if (!res[value.name]) {
            res[value.name] = { name: value.name, price: 0, price: value.price, description: value.description, pic: value.pic };
...
app.get('/contact',(req,res)=>{
    res.locals.items=rew.session.items.length;
    res.render('contact');
});
